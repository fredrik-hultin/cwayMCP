...
class CwayMCPServer:
    ...
    async def _create_per_request_repos(self, token: str):
        """Create repository instances for a single request with given token.
        
        Args:
            token: API token for this request
            
        Returns:
            Tuple of (graphql_client, repositories dict, use_cases dict)
        """
        # Create GraphQL client with request token
        graphql_client = CwayGraphQLClient(api_token=token)
        ...
        use_cases = {
            "kpi": kpi_use_cases,
            "temporal_kpi": temporal_kpi_calculator
        }
        
        return graphql_client, repos, use_cases
    ...
    async def _execute_tool(self, name: str, arguments: Dict[str, Any]) -> Any:
        """Execute a specific tool with per-request authentication.
        
        Uses request context to get the current user's token and creates
        a fresh GraphQL client for this request.
        """
        ...
        request_token = context.authorization_token or settings.cway_api_token
        
        if not request_token:
            return {
                "error": "No authentication token available",
                "message": "Please provide an Authorization header or configure CWAY_API_TOKEN in .env"
            }
        
        # Create per-request GraphQL client and repositories
        graphql_client, repos, use_cases = await self._create_per_request_repos(request_token)
        
        try:
            # Extract repositories and use cases for easier access
            user_repo = repos["user"]
            project_repo = repos["project"]
            artwork_repo = repos["artwork"]
            media_repo = repos["media"]
            share_repo = repos["share"]
            team_repo = repos["team"]
            search_repo = repos["search"]
            category_repo = repos["category"]
            system_repo = repos["system"]
            kpi_use_cases = use_cases["kpi"]
            temporal_kpi_calculator = use_cases["temporal_kpi"]
            ...
            if name == "list_projects":
                # Check if multi-org query requested
                orgs = arguments.get("orgs")
                
                if orgs and context.user_tokens:
                    # Multi-org query: fetch projects from multiple organizations
                    all_projects = []
                    for org_name in orgs:
                        # Get token for this org
                        org_token = context.user_tokens.get(org_name)
                        if not org_token:
                            logger.warning(f"No token found for org '{org_name}', skipping")
                            continue
                        
                        try:
                            # Create client for this org
                            async with CwayGraphQLClient(api_token=org_token) as org_client:
                                org_project_repo = ProjectRepository(org_client)
                                org_projects = await org_project_repo.get_planner_projects()
                                
                                # Add org label to each project
                            ...
                            logger.error(f"Failed to fetch projects for org '{org_name}': {e}")
                    
                    return {
                        "projects": all_projects,
                        "organizations_queried": orgs,
                        "message": f"Fetched {len(all_projects)} projects from {len(orgs)} organizations"
                    }
                else:
                    # Single-org query (current org)
            ...
            # Organization token management tools
            elif name == "register_org_token":
                from ..infrastructure.token_store import get_token_store
                from ..infrastructure.user_identity import get_identity_extractor
                
                # Get current user identity from request context
                if not context.user_identity:
                    return {
                        "error": "No user identity in request context",
                        "message": "Cannot register token without authenticated user context"
                    }
                
                org_name = arguments["org_name"]
                token = arguments["token"]
                
                # Validate the provided token by extracting its identity
                try:
                    identity_extractor = get_identity_extractor()
                    token_identity = await identity_extractor.get_user_identity(token)
                    
                    # Store the token
                    token_store = get_token_store()
                    token_store.store_token(context.user_identity.user_id, org_name, token)
                    
                    return {
                        "success": True,
                        "org_name": org_name,
                        "org_id": token_identity.org_id,
                        "message": f"Token registered for organization '{org_name}' ({token_identity.org_name})"
                    }
                except Exception as e:
                    logger.error(f"Failed to register org token: {e}")
                    return {
                        "success": False,
                        "error": "Invalid token",
                        "message": str(e)
            ...
            elif name == "list_my_organizations":
                from ..infrastructure.token_store import get_token_store
                
                if not context.user_identity:
                    return {
                        "error": "No user identity",
                        "message": "Cannot list organizations without authenticated user"
                    }
                
                token_store = get_token_store()
                org_names = token_store.list_orgs(context.user_identity.user_id)
                
                # Include the primary org from request
                orgs = [{
                    "name": context.user_identity.org_name,
                    "org_id": context.user_identity.org_id,
                    "primary": True
                }]
                
                # Add registered orgs
                for org_name in org_names:
                    if org_name != context.user_identity.org_name:
                        orgs.append({
                            "name": org_name,
                            "org_id": None,  # Don't have org_id for stored tokens
                            "primary": False
                        })
                ...
                return {
                    "organizations": orgs,
                    "count": len(orgs),
                    "message": f"Found {len(orgs)} organizations for user {context.user_identity.user_id}"
                }
            
            elif name == "remove_org_token":
                from ..infrastructure.token_store import get_token_store
                
                if not context.user_identity:
                    return {
                        "error": "No user identity",
                        "message": "Cannot remove token without authenticated user"
                    }
                
                org_name = arguments["org_name"]
                token_store = get_token_store()
                success = token_store.delete_token(context.user_identity.user_id, org_name)
                
                if success:
                    return {
                        "success": True,
                        "org_name": org_name,
                        "message": f"Token for organization '{org_name}' removed successfully"
                    }
                else:
                    return {
                        "success": False,
            ...
            # Always disconnect the per-request GraphQL client
            if graphql_client:
